...
import { Sparkles, CheckCircle2, Zap, ArrowRight, X, ChevronRight, Loader2 } from "lucide-react"
...
interface QuizCard {
    id: string
    card_type: 'flashcard' | 'single_choice' | 'multiple_choice'
    question: string
    answer: string | null
...
export default function GulpPage() {
    const [selectedArticle, setSelectedArticle] = useState<StreamCard | null>(null)
    ...
    const loadStreamCards = async () => {
        try {
            setLoading(true)
            const data = await api.getGulpStream()
            setStreamCards(data as StreamCard[])
        } catch (error: any) {
    ...
    const loadQuizCards = async () => {
        try {
            const data = await api.getQuizCards()
            setQuizCards(data as QuizCard[])
        } catch (error: any) {
            console.error('Failed to load quiz cards:', error)
            alert(error.message || '加载测验卡片失败')
        }
    }

    const currentQuiz = quizCards[currentQuizIndex]
    ...
    const handleNextQuiz = async () => {
        ...
        let isCorrect = false
        if (currentQuiz.card_type === 'flashcard') {
            isCorrect = true // Assume correct for flashcards
        } else if (currentQuiz.card_type === 'single_choice') {
            isCorrect = selectedSingleOption === currentQuiz.correct_answer
        } else if (currentQuiz.card_type === 'multiple_choice') {
            const correctAnswers = currentQuiz.correct_answer as number[]
            isCorrect = selectedMultiOptions.length === correctAnswers.length &&
                selectedMultiOptions.every(opt => correctAnswers.includes(opt))
        }

        try {
            await api.submitQuiz(currentQuiz.id, isCorrect)
        } catch (error) {
    ...
    const renderQuizContent = () => {
                ...
                <div className="flex flex-col h-full w-full max-w-2xl mx-auto">
                    <div className="mb-6 flex items-center justify-between px-2">
                        <Badge variant="outline" className="text-zinc-500 border-zinc-200 bg-white/50 uppercase tracking-wider text-[10px] font-bold">
                            {currentQuiz.topic || 'Flashcard'}
                        </Badge>
                        <span className="text-xs font-bold text-zinc-400">{currentQuizIndex + 1} of {quizCards.length}</span>
                    </div>
                    ...
                    >
                        ...
                        <div className="absolute inset-0 backface-hidden rotate-y-180 flex flex-col items-center justify-center p-10 bg-zinc-900 border border-zinc-800 rounded-[2rem] shadow-2xl">
                            ...
                            {currentQuiz.explanation && (
                                <p className="text-sm text-zinc-400 mt-4 text-center max-w-xl">{currentQuiz.explanation}</p>
                            )}
                            ...
                            <div className="absolute bottom-8 flex gap-4 w-full px-10 justify-center">
                                ...
                                >
                                    {currentQuizIndex === quizCards.length - 1 ? 'Finish' : 'Got it, Next'}
                                </Button>
                            </div>
                        </div>
                    </div>
        ...
        return (
            <div className="flex flex-col h-full w-full max-w-2xl mx-auto">
                <div className="mb-6 flex items-center justify-between px-2">
                    <Badge variant="outline" className="text-zinc-500 border-zinc-200 bg-white/50 uppercase tracking-wider text-[10px] font-bold">
                        {currentQuiz.topic || 'Quiz'} · {currentQuiz.card_type === 'single_choice' ? 'Single Choice' : 'Multiple Choice'}
                    </Badge>
                    <span className="text-xs font-bold text-zinc-400">{currentQuizIndex + 1} of {quizCards.length}</span>
                </div>
                ...
                <div className="flex flex-col h-full bg-white border border-zinc-200 rounded-[2rem] shadow-xl shadow-zinc-200/50 p-10">
                    ...
                    <div className="flex flex-col gap-4 flex-1 overflow-y-auto pr-2 pb-4">
                        {currentQuiz.options?.map((option, index) => {
                            const isSingle = currentQuiz.card_type === 'single_choice'
                            ...
                            if (showQuizResult) {
                                const isCorrectAnswer = isSingle
                                    ...
                                    : (currentQuiz.correct_answer as number[])?.includes(index)
                            ...
                            return (
                                ...
                                >
                                    <div className={`flex items-center justify-center shrink-0 w-7 h-7 rounded-full border-2 ${isSingle ? 'rounded-full' : 'rounded-md'} ${isSelected ? 'border-zinc-900 bg-zinc-900 text-white' : 'border-zinc-300'} ${showQuizResult && (isSingle ? index === currentQuiz.correct_answer : (currentQuiz.correct_answer as number[])?.includes(index)) ? 'border-emerald-500 bg-emerald-500 text-white' : ''}`}>
                                        {isSelected && !showQuizResult && <div className={`w-2.5 h-2.5 bg-white ${isSingle ? 'rounded-full' : 'rounded-sm'}`} />}
                                        {showQuizResult && (isSingle ? index === currentQuiz.correct_answer : (currentQuiz.correct_answer as number[])?.includes(index)) && <CheckCircle2 className="w-5 h-5 text-white" />}
                                        {showQuizResult && isSelected && !(isSingle ? index === currentQuiz.correct_answer : (currentQuiz.correct_answer as number[])?.includes(index)) && <X className="w-5 h-5 text-white" />}
                                    </div>
                                    <span className={`text-[17px] font-semibold leading-tight ${showQuizResult && (isSingle ? index === currentQuiz.correct_answer : (currentQuiz.correct_answer as number[])?.includes(index)) ? 'text-emerald-900' : 'text-zinc-800'}`}>
                                        {option}
                                    </span>
                                </div>
                            )
                    ...
                    {currentQuiz.explanation && showQuizResult && (
                        <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-xl">
                            <p className="text-sm text-blue-900"><strong>Explanation:</strong> {currentQuiz.explanation}</p>
                        </div>
                    )}
                    ...
                    <div className="pt-8 mt-auto flex justify-end">
                        {!showQuizResult ? (
                            <Button
                                ...
                                disabled={currentQuiz.card_type === 'single_choice' ? selectedSingleOption === null : selectedMultiOptions.length === 0}
        ...
        <div className="h-full flex flex-col p-8 bg-transparent max-w-7xl mx-auto w-full relative">
            ...
            <div className="flex items-center justify-between mb-8 pb-6 border-b border-border/50 shrink-0">
                ...
                <Button
                    ...
                    disabled={quizCards.length === 0 && !loading}
                >
                    ...
                    <Badge className="ml-1 bg-white/20 hover:bg-white/20 text-white border-none px-1.5 rounded-full text-[10px] font-bold">{quizCards.length}</Badge>
                </Button>
            </div>
                ...
                <div className="flex-1 overflow-y-auto pb-32 pr-4 custom-scrollbar">
                    {streamCards.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full text-center">
                            <Sparkles className="h-16 w-16 text-zinc-300 mb-4" />
                            <h3 className="text-lg font-semibold text-zinc-700 mb-2">No Content Yet</h3>
                            <p className="text-sm text-zinc-500 mb-6">Add some snapshots in the Feed page to see them here.</p>
                        </div>
                    ) : (
                            ...
                            <div className="p-10 md:p-14 flex flex-col">
                                ...
                                <div className="prose prose-zinc prose-lg max-w-none prose-p:font-medium prose-p:leading-relaxed prose-p:text-zinc-600">
                                    ...
                                    {!selectedArticle.content && !selectedArticle.summary && (
                                        <p className="text-zinc-500">No content available for this article.</p>
                                    )}
                                </div>
                            </div>
                        </div>
