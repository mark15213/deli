...
**导入规则 (Import Rules)：**
支持导入以下格式的文档生成卡片：
1. **CSV**: 第一行为表头。必填字段 `question`。可选字段 `answer`、`type`（默认 `qa`，可选 `mcq`、`cloze`）、`tags` (逗号分隔)、`options` (分号/逗号分隔)、`explanation`。
2. **Markdown/Text**: 自动根据正则提取。支持 `Q: ... A: ...` 行块格式，或 `## 问...` 加下一行答案的标题块格式（提取为普通问答卡片）。
